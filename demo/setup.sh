...
kubectl -n cis-test apply -f app/juiceshop-deployment.yaml
kubectl -n cis-test apply -f app/juiceshop-service.yaml
...
juiceshopServicePort=$(kubectl -n cis-test get svc owasp-juiceshop -o json | jq -r .spec.ports[].port)
# ips
# # container connector
...
cisPod=$(kubectl -n kube-system get pods --field-selector=status.phase=Running -n kube-system -o json | jq -r ".items[].metadata | select(.name | contains (\"k8s-bigip-ctlr-deployment\")).name")
# config-map nodeport
...
read answer
if [ $answer == "yes" ]; then
    cisPod=$(kubectl get pods -n kube-system -o json | jq -r ".items[].metadata | select(.name | contains (\"k8s-bigip-ctlr-deployment\")).name")
